...
void draw() {
  ...
  for (int i = 0; i < faces.length; i++) { 
    ...
    PImage face = get(faces[i].x*scl, faces[i].y*scl, faces[i].width*scl, faces[i].height*scl);
    image(video, 0, 0 );
    image(face, faces[i].x, faces[i].y, faces[i].width, faces[i].height);
    glitch(faces[i].x+faces[i].width/2, faces[i].y+faces[i].height/2);
  }
...
void keyPressed() {  // store the actual image in memory
    if(key == CODED) { 
      if (keyCode == UP){
        print("took screen");
        saveFrame("oi.png");
...
void glitch(int posx, int posy) {
  ...
  for(int i=0;i<500;i++){
    square1_bias = random(-70,70);
    square2_bias = random(-70,70);
    white = random(0,255);
    fill(white, white, white, random(100,255));
    if(dist(posx + square1_bias, posy + square2_bias, posx, posy) < circle_radius){
      rect(posx + square1_bias, posy + square2_bias, random(3,7), random(3,7)); 
    }
  }
}
